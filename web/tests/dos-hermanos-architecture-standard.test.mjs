...
test('Dos Hermanos architecture roots are the canonical frontend structure', () => {
  for (const path of [
    'src/App.tsx',
    'src/core/app/nav.ts',
    ...
    assert.ok(existsSync(path), `Canonical Dos Hermanos source path is missing: ${path}`);
  }
  ...
  assert.deepEqual(symbolicLinks('src'), [], 'Dos Hermanos frontend source must not depend on compatibility symlinks.');
});

test('architectural imports communicate ownership through Dos Hermanos aliases', () => {
  ...
  assert.match(shellSource, /from '@shared\/ui\/AdminShell'/);
...
test('shared UI has one canonical Dos Hermanos implementation tree', () => {
  for (const name of [
    'Header.tsx',
...
test('styles use canonical Dos Hermanos ownership layers and responsive contract stays last', () => {
  ...
  for (const name of canonicalStyles) {
    assert.ok(existsSync(`src/styles/${name}`), `Canonical Dos Hermanos style ownership file is missing: ${name}`);
