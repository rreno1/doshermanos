...
test('management tabs use the canonical GSU twelve-pixel content rhythm', async () => {
  const [styleIndexSource, presentationSource, widgetsSource] = await Promise.all([
  ...
  assert.match(presentationSource, /\.management-page > \.users-section > \.tab-bar/);
  assert.match(presentationSource, /\.management-page > \.audit-section > \.tab-bar/);
