...
export function subscribeToEquipment(
  ...
  return onSnapshot(
    equipmentQuery,
    (snapshot) => onItems(snapshot.docs.map(parseEquipmentItem)),
    onError,
  );
}
...
export function subscribeToEquipmentAssignments(
  ...
  return onSnapshot(
    assignmentsQuery,
    (snapshot) => onAssignments(snapshot.docs.map(parseEquipmentAssignment)),
    onError,
  );
}
...
function parseEquipmentItem(snapshot: DocumentSnapshot): EquipmentItem {
  ...
  return {
    ...
    lastTransactionId: data.lastTransactionId === null ? null : requireString(data.lastTransactionId, 'Equipment history link is invalid.'),
  };
}

function parseEquipmentAssignment(snapshot: DocumentSnapshot): EquipmentAssignment {
  const data = snapshot.data();
  if (!data) {
    throw new Error('Equipment assignment data is unavailable.');
  }
...
function requireTimestamp(value: unknown, message: string): Timestamp {
