...
test('two customers can request events on the same date', async () => {
  const customerADatabase = testEnvironment
    .authenticatedContext('customer-a')
    .firestore();
  const customerBDatabase = testEnvironment
    .authenticatedContext('customer-b')
    .firestore();
...
test('customer can read only their own reservation records', async () => {
  await testEnvironment.withSecurityRulesDisabled(async (context) => {
    const database = context.firestore();
    const timestamp = Timestamp.fromMillis(1_700_000_000_000);
